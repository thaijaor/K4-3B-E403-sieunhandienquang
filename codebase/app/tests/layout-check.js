...
async (page) => {
  ...
  for (const [width, height] of [[1920, 922], [1536, 738], [1280, 615], [1440, 738], [1366, 650], [1024, 600], [800, 500], [720, 600], [375, 667], [320, 568]]) {
    ...
    const measurement = await page.evaluate(() => {
      ...
      return {send:visible('send'), persona:visible('persona-open'), newChat:visible('new-chat'), question:visible('question'), messagesHeight:list.clientHeight, messagesScroll:list.scrollTop, overflow:document.documentElement.scrollWidth > innerWidth};
    });
    if (![measurement.send,measurement.persona,measurement.newChat,measurement.question].every(x=>x.visible) || measurement.overflow || measurement.messagesHeight < 40 || measurement.messagesScroll === 0) throw new Error(`${width}x${height}: ${JSON.stringify(measurement)}`);
