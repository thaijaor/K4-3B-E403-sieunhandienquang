...
async (page) => {
  ...
  check(current.text.startsWith('Ngắn gọn'), 'reject does not mutate persona');
  ...
  check(await page.locator('#undo').count() === 0, 'no undo control');
  await page.getByRole('button', {name: 'Persona', exact: true}).click();
  await page.waitForFunction(() => document.getElementById('persona-read').textContent.includes('Bản đề xuất đã sửa'));
